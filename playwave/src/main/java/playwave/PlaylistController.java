...
public class PlaylistController {
    ...
    private ObservableList<PlaylistTrackController> observable;
    ...
    public void resetTrackList() {
        for (int i = 0; i < playlist.getNumberOfTracks().get(); ++i) {
            try {
                ...
                if (observable.isEmpty()){
                    observable.add(playlistTrackController);
                } else{
                    observable.set(i, playlistTrackController);
                }
        ...
        if (observable.size() > playlist.getNumberOfTracks().get()) {
            observable.remove(playlist.getNumberOfTracks().get(), observable.size());
        }
    }

    public void showPlaylistTracks() throws SQLException {
